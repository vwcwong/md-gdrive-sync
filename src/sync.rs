...
pub fn build(config: &Config, only: &[String]) -> Result<Vec<Document>> {
    ...
    if config.drive.emit_per_repo {
        for section in &sections {
            documents.extend(split::documents_for(
                &section.name,
...
pub fn publish(config: &Config, documents: &[Document], full_sync: bool) -> Result<()> {
